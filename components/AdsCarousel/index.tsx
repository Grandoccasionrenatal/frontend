'use client';
import { advertisementInterface, bannerInterface } from '@/types/api.types';
...
const AdsCarousel = ({ items }: IAdsCarousel) => {
        ...
        {items?.map((i, idx) => (
          ...
          >
            ...
            <div className="absolute px-8 py-8 bg-gradient-to-b from-transparent to-black-1  bottom-0 left-0 w-full  md:rounded-br-custom">
              <h5 className="text-[28px] text-white  md:text-[42px] font-[700] leading-tight">
                {i?.attributes?.title}
              </h5>
              <p className="text-white md:text-[24px] mb-4">{i?.attributes?.description}</p>
            </div>
          </div>
        ))}
      </div>
